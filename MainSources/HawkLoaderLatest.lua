...
if deletefolder then
	deletefolderz = deletefolder
end

if deletefile then
	deletefilez = deletefile
end

if getgenv().listfiles then
	for i,v in pairs(listfilez("Hawk"))do
		if string.find(v, ".webm") then
			deletefilez(v)
		end
	end
end
...
if _Protected_By_Hawk then
	...
	if _Protected_By_Hawk == ".gg/3nfgqgJCqZ" then
		...
		function loadscript()
			...
			Info:Paragraph("Why did Hanki(__spyro) left from team?",{"He has a college login exam 2 years later, in 2025.","He has to study much.","Thats why he left the team.","But don't worry, me, Alex(falpearx) wont let hawk hub die.","Message by falpearx"})

			Info:Label("I will keep Hawk HUB Online!")
			...
			newsec:Image("Ediz","Hawk",{
				Name = "Falpearx", -- "Name of the file and the Title"
				FileName = "Ediz.webm",
				CheckFile = false,
				ImageLink = 13557362289, --webm links or rbxassetid ids only
				ImageColor = Color3.fromRGB(255, 255, 255), --Image Color
				Stroke = Color3.fromRGB(85, 0, 127), --Stroke Color
				Volume = 1, --Volume if you set a video
				Description = {"Scripter Of Hawk HUB","Discord : edizfather"} --Description
			})
			...
			Notifications:Notification("Hawk HUB","Welcome!","Done",1)
		...
		function gargamelsj()
			...
			local Update = game:HttpGet("https://raw.githubusercontent.com/TheHanki/Hawk/main/LastUpdate")
			...
			LastUpdate.Size = UDim2.new(0, 130, 0, 45)
			LastUpdate.ZIndex = 2
			LastUpdate.Font = Enum.Font.GothamBlack
			LastUpdate.Text = Update
			...
			DirectJoinDiscord.MouseButton1Down:Connect(function()
			...
			NoteMessage.Text = "We're using Key System. Because it helps a lot to developers."
			...
			InputBox:GetPropertyChangedSignal("Text"):Connect(function()
				if table.find(Gargamel,InputBox.Text) or InputBox.Text == "hawkhubneverdies" then
					writefile("Hawk\\Key.txt",InputBox.Text)
					loadscript()				
				end
			end)
		...
		elseif isfolder("Hawk") then
			if isfile("Hawk\\Key.txt") then
				if not table.find(Gargamel,readfile("Hawk\\Key.txt")) then
					warn("Wrong key bich")
					gargamelsj()				
				else
					loadscript()				

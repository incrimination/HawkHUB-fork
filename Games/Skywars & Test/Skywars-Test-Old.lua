...
Notificatin:Notification("Place Found!","Skywars","Done",1)
...
local Window = Hawk:Window("Hawk HUB | Skywars & 0_0 Test")

Hawk:Bildirim("Hawk HUB","Script is loaded, Have FUN!",1)
...
HomeTab:Button("Infinite Yield",function()
...
_G.UWU = false
Undermines:Toggle("Undermine Aura", function(value)
	if value == true then
		_G.NoBugs = false
...
_G.breh = false
Undermines:Toggle("Undermine Ore Aura", function(value)
	if value == true then
...
Undermines:Button("Barrier", function()
	wait()
	function onTouched(part)       
		local h = part
		if h.Name == "Block" then
...
_G.Coin = true
HomeTab:Toggle("Collect Coins", function(value)
	if value == true then
		_G.Coin = true
...
--Infinite Jump
infinjump = false
HomeTab:Toggle("Infinite Jump",  function(value)
	infinjump = value
...
HomeTab:Toggle("Noclip",  function(value)
	if value == true then
		noclip()
	elseif value == false then
		clip()
...
HomeTab:Toggle("Esp",function(esp)
	if esp == true then
...
ScriptTab:Button("Reach Gui (Veso Lux)",function()
...
AimsTab:Label("Aims")
AimsTab:Toggle("Bow Trigger-Bot", function(value)
	if value == false then
...
HomeTab:Toggle("Auto Heal (Needs Heal)",function(XD)
	if XD == true then
		while wait() do
...
ScriptTab:Toggle("Anti Void",function(adasadas)
...
ScriptTab:Toggle("Fe Slowmotion",function(adasadas)
...
ScriptTab:Toggle("Realistic Mode",function(value)
...
_G.glitch = false
ScriptTab:Toggle("Glitch Power Ups",function(togil)
	if togil == true then
		_G.glitch = true
...
ScriptTab:Button("Auto Clicker",function(lolz)
...
_Air = false
Visuals:Toggle("Float",function(togil)
	if togil == true then
...
_AirBlock = false
Visuals:Toggle("Air Block",function(togil)
	if togil == true then
		_AirBlock = true
...
Visuals:Button("Fling Players In Round", function()
...
Visuals:Toggle("Ore Esp", function(value)
	if value == true then
		local CURRENTMAP = ""
...
CreditsTab:Label("Scripter / Owner: h4nk1")
...
getgenv().Fov = 0
zorrrd = false
AimsTab:Toggle("Silent Aim",function(zort)
	if zort == true then
		zorrrd = true
		getgenv().Fov = 450
...
if wasted then
	print("Anticheat Bypasser is already running ;)")
else		
	pcall(function()
		getgenv().wasted = true
	end)
	...
	HomeTab:Toggle("Fly",function(omegad)
		if omegad == true then
			sFLY(true)
	...
	local returnFalse = {"BodyVelocity", "BodyPosition", "BodyThrust"}
	...
	old = hookmetamethod(game, "__namecall", function(self, ...)
		...
		if not checkcaller() and method == 'IsA' and table.find(returnFalse, args[1]) or table.find(returnFalse, args[2]) or table.find(returnFalse, args[3]) then
			print("Body Check Attempted!")
			return wait(9e9)
		end
		return old(self, ...)
	end)
	...
	local resp = game:GetService("ReplicatedStorage").Remotes.RespawnServer
	local remote;
	remote = hookmetamethod(game, "__namecall",function(self, ...)
		local args = {...}
		local method = getnamecallmethod();

		if not checkcaller() and self == resp and method == "FireServer" then --> InvokeServer can be used too.
			print("Ac tried to kill u!")
			wait(9e9)
			return
		end;
	...
	local char = game:GetService("Players").LocalPlayer.Character
	local oldpos;
	oldpos = hookmetamethod(game, "__index",function(obj, idx)
		if not checkrole then
			if not checkcaller() and obj == humpart and string.lower(idx) == "position" then
				return Vector3.new(humpart.Position.X,humpart.Position.Y*180,humpart.Position.Z)
			end

			return oldpos(obj, idx)
		end
	end)
	...
	bypass = hookmetamethod(game, "__namecall",function(self, ...)
		...
		if not checkcaller() and (method == "GetState" and self == human) then
			return freefall
		end;
	...
	print("Bypassed __changestate")
	local antichange;
	antichange = hookmetamethod(game, "__namecall",function(self, ...)
		local args = {...}
		local method = getnamecallmethod();

		if not checkcaller() and (method == "ChangeState" and self == human) and tonumber(args[1]) == 5 then
			print("ac tried to kill u cuz youre not falling rn")
			args[1] = 7;
			return wait(9e9)
		end;

		return antichange(self, ...)
	end)
	...
	mt.__namecall = newcclosure(function(self, ...)
		...
		if method == "FireServer" and tostring(self) == "RespawnServer" or tostring(self) == "RemoteEvent" or tostring(self) == "RemoteEvent1" or tostring(self) == "RemoteEvent2" then
			return
		end
	...
	print("Bypassed __statechanged")
	AntiGH = true
	Client = plr
	function
		StateChangedEvent(T, Changed)
		if AntiGH == true then
	...
	humpart = game.Players.LocalPlayer.Character:WaitForChild("HumanoidRootPart")
